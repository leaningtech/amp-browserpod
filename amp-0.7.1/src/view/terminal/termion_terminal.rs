...
use self::termion::event::{parse_event, Event as TermionEvent};
...
impl Terminal for TermionTerminal {
    fn listen(&self) -> Option<Event> {
        ...
        if let Some(event) = events.iter().next() {
            match event.token() {
                STDIN_INPUT => {
                    let mut buffer = self.input.lock().ok()?;

                    let mut chunk = [0u8; 1024];
                    let count = stdin().lock().read(&mut chunk).ok()?;
                    if count == 0 {
                        return None;
                    }
                    buffer.extend_from_slice(&chunk[..count]);

                    let key = match take_key(&mut buffer)? {
                        TermionEvent::Key(key) => key,
                        _ => return None,
                    };
            ...
            // Flush a partial sequence.
            let mut buffer = self.input.lock().ok()?;
            if buffer.is_empty() {
                return None;
            }
            match flush_key(&mut buffer)? {
                TermionEvent::Key(key) => map_key(key),
                _ => None,
            }
        }
    }
...
// Yields bytes, counting them.
struct Consumed<'a> {
    bytes: &'a [u8],
    taken: usize,
}

impl Iterator for Consumed<'_> {
    type Item = ::std::result::Result<u8, ::std::io::Error>;

    fn next(&mut self) -> Option<Self::Item> {
        let byte = *self.bytes.get(self.taken)?;
        self.taken += 1;
        Some(Ok(byte))
    }
}

// Keeps partial sequences.
fn take_key(buffer: &mut Vec<u8>) -> Option<TermionEvent> {
    if buffer.is_empty() {
        return None;
    }
    ...
    let (parsed, taken) = {
        ...
        let parsed = parse_event(first, &mut rest);
        (parsed, rest.taken)
    };

    match parsed {
        Ok(event) => {
            buffer.drain(..1 + taken);
            Some(event)
        }
        // Ran out mid sequence.
        Err(_) if taken == buffer.len() - 1 => None,
        Err(_) => {
            buffer.drain(..1);
            None
        }
...
fn flush_key(buffer: &mut Vec<u8>) -> Option<TermionEvent> {
    if let Some(event) = take_key(buffer) {
        return Some(event);
    }
    if buffer.is_empty() {
        return None;
    ...
    if first == 0x1b {
        return Some(TermionEvent::Key(TermionKey::Esc));
    }

    Some(TermionEvent::Key(TermionKey::Char(first as char)))
}

fn map_key(key: TermionKey) -> Option<Event> {
    match key {
