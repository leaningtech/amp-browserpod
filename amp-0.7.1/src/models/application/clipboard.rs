...
/// There is no OS-level clipboard inside a Pod, so we stand in for the
/// cli-clipboard API and always fail to acquire a context. Copy/paste then
/// runs entirely through the in-app clipboard below.
#[cfg(target_vendor = "browserpod")]
mod cli_clipboard {
    ...
    impl ClipboardProvider for ClipboardContext {
        ...
        fn set_contents(&mut self, _content: String) -> Result<(), ()> {
            Ok(())
        }
    }
}
